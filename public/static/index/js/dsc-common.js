...
$(function () {
  ...
  $("*[ectype='items'] *[ectype='item']").on('mouseenter', function () {
    ...
    if (eveval != 1) {
      ...
      $.ajax({
        type: "POST",
        url: "/index.php/index/Category/getCategotyParentTree",
        data: {"cat_id": cat_id},
        dataType: 'json',
        success: function (data) {
          var str = '';
          var brandStr = '';
          $.each(data.data.cate, function (i, el) {
            str += '<li><a href="#">' + el.cate_name + '</a></li>'
          })
          $.each(data.data.brand, function (i, el) {
            brandStr += '<li><a href="'+el.brand_url+'" target="_black"><img src="'+el.brand_img+'" width="112"height="49"></a></li>'
          })
          $(".cate-list-" + data.cat_id).html(str);
          $(".cate-logo-" + data.cat_id).html(brandStr);
        }
      });
    }
